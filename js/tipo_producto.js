...
var idempresa =$("#txt_idempresa").val();
     t_tipo_producto = $("#tabla_tipo_producto").DataTable({
            "ordering":false,   
        "pageLength":10,
        "destroy":true,
        "async": false ,
    ...
    t_tipo_producto.on( 'draw.dt', function () {
        var PageInfo = $('#tabla_tipo_producto').DataTable().page.info();
        t_tipo_producto.column(0, { page: 'current' }).nodes().each( function (cell, i) {
 ...
 // modificar datos del procedimiento
    $('#tabla_tipo_producto').on('click','.editar',function(){
        var data = t_tipo_producto.row($(this).parents('tr')).data();

         if(t_tipo_producto.row(this).child.isShown()){
                var data = t_tipo_producto.row(this).data();
            }
