...
function listar_valor_inventario() {
    ...
    t_valor_inventario.on('draw.dt', function () {
        var PageInfo = $('#tabla_valor_producto').DataTable().page.info();
        t_valor_inventario.column(0, { page: 'current' }).nodes().each(function (cell, i) {
            cell.innerHTML = i + 1 + PageInfo.start;
        });
    });
