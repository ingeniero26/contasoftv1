...
class Modelo_Bodegas{
	...
	function Registrar_Bodega($nombre_bodega,$idempresa) {
		...
		$sql = "call SP_MODIFICAR_ESTATUS_BODEGA('$id','$estatus')";
			...
			function listar_combo_bodegas($idempresa) {
				$sql = "SELECT id,nombre_bodega FROM bodega
		WHERE  bodega.`idempresa` = '$idempresa' AND bodega.`estatus` = 'ACTIVO'";
				$arreglo = array();
			if($consulta = $this->conexion->conexion->query($sql)){
