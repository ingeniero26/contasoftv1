<?php 
 ...
 class Modelo_Ingreso {
	...
	function listar_ingreso($finicio,$ffin,$idempresa){
   ...
   FROM
    ...
    INNER JOIN `persona`     ON (`proveedor`.`persona_id` = `persona`.`persona_id`)
     ...
     WHERE compra.compra_fecha BETWEEN '$finicio' AND  '$ffin' AND compra.`idempresa` = '$idempresa'";
		...
		function listar_combo_proveedor($idempresa) {
	...
	function listar_combo_producto($idempresa) {
	...
	function Registrar_Compra($idproveedor, $idbodega, $idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante , $tipo_pago,$fecha_compra,$impuesto ,$total,$estado,$porcentaje,$decto,$fecha_vencimiento,$idempresa) {
